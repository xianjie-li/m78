...
const Scroll = React.forwardRef<ScrollRef, ScrollProps>(({
  ...
  /* 控制返回顶部开关 */
  const debounceOnScroll = useCallback(_debounce((event: FullGestureState<Coordinates>) => {
  ...
  const bind = useGesture({
    onDrag({ event, down, cancel, first, movement: [, moveY] }) {
      ...
      if (atTop && !down) {
        ...
        // 大于阈值触发刷新动作
        if (spPullDown.y.getValue() >= pullDownThreshold - 10) { // 使用实时值进行判断防止快速滑动导致的误触, -10可以增强体验，因为动画存在一定的延迟
        // if (moveY >= pullDownThreshold) {
  ...
  return (
    <div className={cls('fr-scroll_wrap', className)} style={style}>
      ...
      >
        ...
        <If when={isNumber(state.dataLength) && state.dataLength > 0}>获取到{state.dataLength}条{self.loadCount === 1 ? '' : '新'}数据</If>
      </Transition>
      {/* 返回顶部按钮 */}
      ...
      <Toggle when={pullDown}>
        <div className="fr-scroll_pulldown-wrap">
          <animated.div
            ...
            style={{
              transform: interpolate([spPullDown.y, spPullDown.over], (y, over) => {
                return `translateY(${y * 0.8}px) rotate3d(0,0,1,${-(y * 3 + over)}deg)`;
              }),
            }}
          >
            <svg
      ...
      >
        ...
        {/* 上拉加载相关 */}
        <If when={pullUp}>
          <If when={!hasData}><div style={{ height: '26%' }} /></If> {/* 撑开高度以优化显示 */}
          <If when={!hasData && self.loadCount !== 0 && !state.pullUpLoading && !state.pullUpHasError}>
            <Empty desc="暂无数据" size="large">
              <Button size="small" onClick={onReTry}>再试试</Button>
            </Empty>
          </If>
          <div className="fr-scroll_pullup-wrap">
            <Switch>
              ...
              <If when={state.dataLength !== 0 && hasData && !state.pullUpLoading && !state.pullDownLoading}>
                <span className="fr-scroll_loadmore fr-scroll_tip-base" onClick={() => touchBottom()}>
