import { AnyFunction, AnyObject, CustomEvent, EmptyFunction, NameItem, NamePath } from "@m78/utils";
...
export interface FormValidator {
    (meta: FormVerifyMeta): Promise<void | FormErrorTemplateType | FormErrorTemplateInterpolate> | void | FormErrorTemplateType | FormErrorTemplateInterpolate;
    /** 可选的验证器标识，用于辅助识别 */
    key?: string;
...
export interface FormInstance extends FormVerifyInstance {
     ...
     * */
    verify(name?: NamePath, extraMeta?: AnyObject): Promise<FormRejectMeta | null>;
    /**
     * debounce版本的verify, 处理高频调用时可以使用, cb会在成功或失败时触发, 失败时包含错误信息
     *
     * 注意: 由于防抖机制, 连续调用时, 大部分验证都会被忽略, 所以cb不是必定触发的, 通常只有第一次和最后一次调用触发
     * */
    debounceVerify: (name?: NamePath, cb?: (error?: FormRejectMeta) => void) => void;
    /**
    ...
    events: {
        ...
        fail: CustomEvent<(errors: FormRejectMeta, isValueChangeTrigger?: boolean) => void>;
        /** 重置事件 */
...
export interface FormSchema {
    ...
    valid?: boolean;
    /** 动态设置其他参数 */
    dynamic?: (form: FormVerifyInstance) => Omit<FormSchemaWithoutName, "dynamic" | "name" | "list"> | void;
    /** 类型为数组、对象时, 对其结构进行验证 */
    schema?: FormSchema[];
...
export interface _Context {
    ...
    /** 是否处于verify模式 */
    verifyOnly: boolean;
    /** 获取当前schema并处理dynamic等项, 返回处理后的schema和所有无效项的name数组 */
    getFormatterSchemas(): [FormSchemaWithoutName, NamePath[]];
    /** 对Schema上的dynamic进行处理, 并克隆validator */
    schemaBaseHandleAndClone(schema: FormSchemaWithoutName | FormSchema): void;
    /** 执行静态schema验证 */
    schemaCheck: FormVerify["check"];
}
