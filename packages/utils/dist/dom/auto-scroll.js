...
/** 一个光标在目标边缘时自动滚动节点的工具 */ export function createAutoScroll(config) {
    var getAutoScrollStatus = /** 获取光标和目标位置的边缘覆盖状态 */ function getAutoScrollStatus(param, isLast) {
   ...
   * */ function autoScrollByStatus(status) {
    ...
    var trigger = function trigger(xy, isLast, conf) {
    ...
    return {
        ...
        /** 更新配置, 后续trigger以合并后的配置进行 */ updateConfig: function(newConf) {
