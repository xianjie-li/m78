 ...
 * */ export var PhysicalScroll = /*#__PURE__*/ (function () {
  ...
  function PhysicalScroll(config) {
    ...
    this.start = function (e) {
      var _xy = _sliced_to_array(e.xy, 2),
        clientX = _xy[0],
        clientY = _xy[1];
    ...
    this.move = function (e) {
      ...
      var ref1 = _sliced_to_array(_this.getScrollPosition(), 2),
        x = ref1[0],
        y = ref1[1];
      x = x - offsetX;
      y = y - offsetY;
      ...
      (ref = (_config = _this.config).onScroll) === null || ref === void 0
        ? void 0
        : ref.call(_config, [x, y], false);
      var now = Date.now();
      _this.moveXList.push([offsetX, now]);
      _this.moveYList.push([offsetY, now]);
      _this.prevX = clientX;
      _this.prevY = clientY;
    };
    this.end = function (e) {
      ...
      var endXSvgMove = Math.abs(
        _this.getSampleOffset(sampleTimeStart, _this.moveXList) || 0
      );
      var endYSvgMove = Math.abs(
        _this.getSampleOffset(sampleTimeStart, _this.moveYList) || 0
      );
      if (
        duration > lightSweep ||
  ...
  /** 根据对应轴的移动距离和持续时间执行自动滚动 */ _proto.autoScroll =
    function autoScroll(xDistance, yDistance, duration) {
      var _this = this;
      this.rafClear = raf(function (t) {
        ...
        // 这里需要同步更新滚动位置
        if (!_this.config.onlyNotify) {
          _this.setScrollPosition([x, y], true);
        }
        (ref = (_config = _this.config).onScroll) === null || ref === void 0
          ? void 0
          : ref.call(_config, [x, y], true);
  ...
  /** 根据一组[offset, time]和提供的起始时间获取该时间之后移动距离的平均值, 如果最后一段时间未移动, 可能返回undefined */ _proto.getSampleOffset =
    function getSampleOffset(startTime, list) {
      var svg;
      for (var i = list.length - 1; i >= 0; i--) {
        var _i = _sliced_to_array(list[i], 2),
          offset = _i[0],
          time = _i[1];
        if (time < startTime) {
          if (svg !== undefined) {
