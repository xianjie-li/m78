 ...
 * */
export declare class TablePlugin {
    /** table实例 */
    table: TableInstance;
    /** 当前注册的所有plugin */
    plugins: TablePlugin[];
     ...
     * */
    methodMapper<T extends object = AnyObject>(obj: T, methods: (keyof T | [string, keyof T])[]): void;
    /** 获取指定插件类的实例, 不存在对应类型的插件时, 抛出异常 */
    getPlugin<T extends TablePlugin = any>(pluginClass: new (...args: any[]) => T): T;
}
/** 初始化/reload操作的一些主要阶段, 枚举的key声明顺序与其执行顺序有关 */
export declare enum TableLoadStage {
    /** 执行full reload */
