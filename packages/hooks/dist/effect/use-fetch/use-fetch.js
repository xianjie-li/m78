 ...
 * */ function useFetch(method) {
    ...
    var initData = options.initData, initPayload = options.initPayload, _options_deps = options.deps, deps = _options_deps === void 0 ? [] : _options_deps, param = options.param, _options_manual = options.manual, manual = _options_manual === void 0 ? false : _options_manual, _options_timeout = options.timeout, timeout = _options_timeout === void 0 ? 10000 : _options_timeout, onSuccess = options.onSuccess, onError = options.onError, onFinish = options.onFinish, onTimeout = options.onTimeout, cacheKey = options.cacheKey, _options_stale = options.stale, stale = _options_stale === void 0 ? true : _options_stale, throttleInterval = options.throttleInterval, debounceInterval = options.debounceInterval, tmp = options.pass, aPass = tmp === void 0 ? true : tmp;
    ...
    var _useStorageState = _sliced_to_array(useStorageState("".concat(cacheKey, "_FETCH_PAYLOAD"), initPayload, {
        disabled: !cacheEnable
    }), 2), payload = _useStorageState[0], setPayload = _useStorageState[1];
    var _useStorageState1 = _sliced_to_array(useStorageState("".concat(cacheKey, "_FETCH_DATA"), initData, {
        disabled: !cacheEnable
    }), 2), data = _useStorageState1[0], setData = _useStorageState1[1];
    ...
    /** 接受可选的新payload，并根据条件返回传递给fetchHandel的参数(使用param或payload) */ function getActualPayload(newPayload) {
    ...
    /** 返回一个将互斥的状态还原的对象，并通过键值覆盖设置某个值 */ function getResetState(key, value) {
        var resetState = {
            loading: false,
            error: undefined,
            timeout: false
