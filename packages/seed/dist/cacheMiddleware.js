...
/** 设置缓存时间，默认只在未设置时设置，开启expireRefresh后在每一次执行时更新缓存 */ function setExpire(
  expireKey,
  storage,
  config
) {
...
/** 检测缓存有效性，过期时删除缓存，启用expireRefresh且未过期时刷新缓存, 否则不执行操作 */ function checkExpire(
  k,
  expireKey,
  storage,
  config
  ...
  if (Date.now() > Number(exT)) {
    var ref;
    (ref = config.onExpire) === null || ref === void 0
      ? void 0
      : ref.call(config);
