...
实现表单功能, 注重于简化表单中难以处理的部分, 如: 值收集/验证/联动/动态表单等
...
- 如果你对样式有较强的定制医院, 可以手动渲染 `Field` 组件.
...
```tsx
// bind提供了一些列需要绑定到表单控件上的prop, binder用于给指定react节点绑定props, element为component中配置的组件
...
```tsx
<Field component={<input />} adaptor={inputAdapter} />
```
...
```tsx
<Field component={<MyInput placeholder="请输入" />} />

// 如果设置了字符串name, 还可以通过字符串直接使用组件
<Field component="MyInput" componentProps={{ placeholder: "请输入" }} />
```

### 内置表单控件

`m78` 的所有内置表单控件均无需编写适配器, 可以直接在 `Form` 或 `Table` 中使用, 此外, 在没有提供适配器时, 会使用默认适配器, 像 `<input />` 这样的简单的原生表单控件通常不需要编写适配器.

## 嵌套结构

如果表单值嵌套在另一个对象中, 可通过 `name` 指定嵌套的路径.
...
> 使用函数验证器而不是以往的字符串验证的原因是: 更好的类型检测, 清晰的来源, 无 DSL 和几乎为 0 的学习成本.
...
异步验证器的唯一区别是他返回一个 Promise, Promise 的结果字符串或内部抛出的错误会被视为验证错误.

### @m78/verify

核心的验证逻辑抽象在 [@m78/verify](https://github.com/xianjie-li/m78/tree/master/packages/verify) 包中, 如果你需要对验证细节的更进一步掌控, 或者了解更多验证器的使用方式, 可以参考这个包的文档
...
- Filed.component 可接收一个函数, 其与适配器签名完全一致, 可以用来临时进行局部的定制

另外, 在 form config / schema / Field 中提供了很多用于样式微调的配置, 具体可查看对应文档.
