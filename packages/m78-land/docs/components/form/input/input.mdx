...
<Demo
  demo={require("./_input-demo.tsx")}
  code={require("!!raw-loader!./_input-demo.tsx")}
/>

## 数值输入

<Demo
  demo={require("./_input-num-demo.tsx")}
  code={require("!!raw-loader!./_input-num-demo.tsx")}
/>

## 拦截器

拦截器用于在字符值发生改变之前对其进行阻断或变更, 其具有以下 API

> 在一些场景中, 你需要字符长度被拦截变更时手动修正光标的位置, 拦截器提供了便捷的 api 用于操作光标
...
作为例子, `type=integer`时使用了以下两个内置拦截器

```ts
const _number: Interceptor = ({ str }) => {
  if (str === "-") return str;
...
const _integer: Interceptor = ({ str }) => {
...
支持 受控/非受控 使用, 具体见 [受控/非受控](/docs/components/other/control-uncontrol)
...
interface InputProps extends FormLike<string>, InputPropsBase {
  /* # # # # # # # 功能 # # # # # # # */
  /**
