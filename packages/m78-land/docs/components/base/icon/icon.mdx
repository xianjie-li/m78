...
import IconDemo from "./_icon-demo.tsx";
...
> 如果你不建议幻影依赖, 因为`@m78/icons`同时也是`m78`库的依赖
...
> 没有提供统一的出口, 因为图标总数约 4k+, 即使你的打包器支持 tree shake, 导入分析也会对你的智能编辑器造成巨大的负担, 严重影响开发体验.
...
`icon`没有提供任何特别的配置项, 你可以通过[layout](/component/base/layout)中提供的原子类像对待一个常规 dom 节点一样对其进行定制
...
如果内置图标中缺少你想要的, 你可以随意添加任意来源的图标, 比如字体图标, svg 图标, 图片等, 但是为了和内置图标样式保持统一, 可以选择为它们添加`m78-icon`类名
